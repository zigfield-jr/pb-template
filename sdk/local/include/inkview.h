...
#define VARRUNDIR DIRPREFIX "/var/run"
#else
...
#define VARRUNDIR "./system/var/run"
#endif
...
#define READER_CACHE_APP_PATH SYSTEMDATA "/bin/eink-cache-reader.app"
...
#define TRACK_METADATA_CONFIG "/tmp/track_metadata.cfg"
...
// sockets names
#define READER_MANAGER_SOCKET_NAME "\0reader_manager_socket_name"
#define READER_CACHE_SOCKET_NAME "\0reader_cache_socket_name"

#define AUDIOBOOKS_DEFAULT_PATH FLASHDIR "/Audio Books"
#define AUDIOBOOKS_CONFIG_DIRECTORY CONFIGPATH "/audiobooks"
#define AUDIOBOOKS_CONFIG_FILE AUDIOBOOKS_CONFIG_DIRECTORY "/audiobooks.cfg"

#define IMAGES_PARAM_PATH CONFIGPATH "/images_parameters.cfg"
#define SYSTEM_IMAGES_PARAM_PATH SYSTEMDATA "/config/images_parameters.cfg"

#ifdef EMULATOR
#define USER_MENU_ICON_PATH "/tmp/bookstore_covers_cache/UserLoginIcon.png"
#else
#define USER_MENU_ICON_PATH CONFIGPATH"/UserLoginIcon.png"
#endif

#define BOOKSTORE2_APP_PATH SYSTEMDATA "/bin/bookstore-2.app"
#define BOOKSTORE2_CONFIG CONFIGPATH "/bookstore.cfg"

#define BOOKSTORE2_FEEDS_CACHE_NAME "bookstore_feeds_cache"
#define BOOKSTORE2_FEEDS_CACHE_PATH CACHEPATH "/" BOOKSTORE2_FEEDS_CACHE_NAME
#define BOOKSTORE2_COVERS_CACHE_NAME "bookstore_covers_cache"
#define BOOKSTORE2_COVERS_CACHE_PATH CACHEPATH "/" BOOKSTORE2_COVERS_CACHE_NAME

#define BOOKSTORE2_DESKTOP_DATA_CONFIG CONFIGPATH "/desktop_data.cfg"
#define BOOKSTORE2_DESKTOP_DATA_CACHE_PATH CACHEPATH "/bookstore_desktop_data_cache"

#define OPTIMIZED_BOOKS_PATH STATEPATH "/optimize"
#define USBSTOR_SN_FILE "/var/run/usb_storage_sn"
...
#define MSG_LAUNCH_APP 0x7e000000
#define MSG_OK 1
#define MSG_ERROR 0
...
#define MSG_CALIBRATE_TP	0x126
#define MSG_DROP_CACHES         0x127
#define MSG_SHARE_DEVICE_CFG  0x128
#define MSG_WRITE_LOW_POWER_LOGO         0x129
#define MSG_DOSFSCK     0x12a
...
#define EVT_KEYPRESS_EXT 40
#define EVT_KEYRELEASE_EXT 41
#define EVT_KEYREPEAT_EXT 42
...
#define EVT_TOUCHUP   47
#define EVT_TOUCHDOWN 48
#define EVT_TOUCHMOVE 49
...
#define EVT_USBSTORE_IN		163
#define EVT_USBSTORE_OUT	164
...
#define EVT_ASYNC_TASK_FINISHED    262 // used for framework-2 async_code realization

#define EVT_STOP_PLAYING 263
#define EVT_AVRCP_COMMAND 264
...
#define TASK_GROUP1                     (1 << 8)
#define TASK_GROUP2                     (1 << 9)
#define TASK_OOMPROOF                   (1 << 14)
#define TASK_AUTORESTART                (1 << 15)
#define TASK_DONTSENDTASKMSG            (1 << 16)
#define TASK_LOW_PRIORITY_FOR_OOM_KILL  (1 << 17)
#define TASK_HIGH_PRIORITY_FOR_OOM_KILL (1 << 18)

#define TASK_FB_MASK                    (3 << 20)
#define TASK_FB_GS8                     (0 << 20)
#define TASK_FB_RGB24                   (1 << 20)

#define TASK_SELFREGISTER               (1 << 30)
...
// Text output commands after 0x08
#define TEXT_OUTPUT_COMMAND 0x08
#define TEXT_OUTPUT_CMD_LINEBREAK_ON_SPACE_ONLY_ON      0x01
#define TEXT_OUTPUT_CMD_LINEBREAK_ON_SPACE_ONLY_OFF     0x02
#define TEXT_OUTPUT_CMD_STRIKETHROUGH_TEXT_ON           0x03
#define TEXT_OUTPUT_CMD_STRIKETHROUGH_TEXT_OFF          0x04
...
// removed obsolete hwdepth parameter, now hwdepth=4 for all controllers
...
enum iepub3data_type_s {
    ...
    kEPUB3TitleTypeUnknown      = 7,
    kEPUB3TitleTypeMAX,

    kEPUB3AuthorType            = 101,

    kEPUB3SeriesType            = 201,
};
...
typedef struct iv_fbinfo_s {
    ...
    int vdepth;
    int orientation;
    int scanline_portrait;
    int scanline_landscape;
    int scanline;
    int fb_offset;
    int fb_size;
    int ev_offset;
...
////input_dev_e is touch 'devtype'; it describes input device.
//enum input_dev_e {
//	TP_UNKNOWN = 0,
//	TP_CAPACITIVE,
...
typedef iv_mtinfo iv_mtinfo_section[MT_SECTION_LEN];
...
typedef enum bt_mode_e {
	BT_MODE_OFF = BLUETOOTH_OFF,
	BT_MODE_HIDDEN = BLUETOOTH_HIDDEN,
	BT_MODE_VISIBLE = BLUETOOTH_VISIBLE
} bt_mode;

typedef enum bt_state_e {
	BT_STATE_OFF = 0,
	BT_STATE_SUSPENDED, //SW stack loaded, driver in suspend mode or unloaded
	BT_STATE_READY, //SW stack and driver loaded
	BT_STATE_CONNECTED,
...
//ivmpc->bt_flags bit fields
typedef enum bt_flags_e {
...
typedef struct {
	audio_output_t type;
	char description[32]; //name of audio output connection, like "hp" or mac address for type bt in format xx:xx:xx:xx:xx:xx
} audio_output_info;
...
int SetGSensorEnabled(bool onOff);
bool IsGSensorEnabled();
//int SetGsensorMode(int mode);
...
enum estyle {
    ...
    ROUND_DRAW_CIRCLE_BLACK_WHITE = 256, // draw all circle pixels black or white
...
enum HorizontalSeparatorType {
    HORIZONTAL_SEPARATOR_SOLID,
    HORIZONTAL_SEPARATOR_DOTTED,
};
void DrawHorizontalSeparator(int x0, int y, int w, int type);
...
/**
 * @brief processCircle - call cb for each circle line once (order of lines is not specified).
 ...
 */
void processCircle(int center_x, int center_y, int radius, void* context, void (*cb)(void*, int, int, int, int, int));

void invertCircle(int center_x, int center_y, int radius, icanvas* canvas);
...
void DrawFrameDialog(int x, int y, int w, int h, ibitmap *icon, char *title, int flags, irect *clientarea);
...
/**
 * @brief TransparentGradientData, TransparentGradient, TransparentGradientRect - Draw gradient transparent
 * @param x
 * @param y
 * @param w
 * @param h
 ...
 */
void TransparentGradientData(unsigned char *data, int depth, int scanline, int x, int y, int w, int h, /*egradient*/int direction);
...
typedef enum ImageSignatureType_e {
    kImgTypeUnknown,
    kImgTypeBMP,
    kImgTypeGIF,
    kImgTypeJPG,
...
ImageSignatureType GetImageType(const char *path);
ibitmap *LoadImageToFormat(const char *path, PixelFormat format);

ibitmap *LoadGIFToFormat(const char *path, PixelFormat format);
ibitmap *LoadPNGToFormat(const char *path, PixelFormat format);
ibitmap *LoadJPEGToFormat(const char *path, PixelFormat format);
ibitmap *LoadTiffToFormat(const char *path, PixelFormat format);

ibitmap *NewBitmapWithFormat(int width, int height, PixelFormat format);

ibitmap *LoadPNG8(const char *path);
ibitmap *LoadJPEG8(const char *path);
ibitmap *NewBitmap8(int width, int height);
ibitmap *NewBitmap24(int width, int height);
...
void ApplyContrastToBitmap8(ibitmap* bmp, int contrast);
/**
...
ibitmap * BitmapStretchProportionally(const ibitmap * bmp, int width, int height);

ibitmap * BitmapStretchFill(const ibitmap * bmp, int width, int height);


/**
 * @brief adjustBitmapDefault make default ajustment of bitmap with adjustBitmapWithSaturationBrightnessAndDithering
 * @param bmp - bitmap to make adjustment
 */
void adjustBitmapDefault(ibitmap* bmp);

/**
 * @brief adjustBitmapWithSaturationBrightnessAndDithering - change saturation and value of bitmap pixels (as in HVS color representation)
 ...
 */
void adjustBitmapWithSaturationBrightnessAndDithering(ibitmap* bmp, int * saturation_table, int * value_table, bool dithering);
void adjustAreaWithSaturationBrightnessAndDithering(unsigned char* data, int scanline, int width, int height, int * saturation_table,
                                                    int * value_table, bool dithering);
...
typedef struct AdjustmentParams_s {
    double lab_saturation_koef;
    double vibrance;
    int saturation_table[256];
    int value_table[256];
    double brightness;
    double contrast;
    double gamma;
...
void ditherAndAdjustArea(unsigned char* data, int scanline, int width, int height, AdjustmentParams* params);


void adjustAreaWithBrightnessContrastGamma(unsigned char * data, int scanline, int width, int height, double brightness, double contrast, double gamma);

void getBrightnessContrastGammaAdjustParams(int *brightness, int *contrast, int *gamma);

void bm_stretch8(unsigned char *in, int ix, int iy, int iw, int ih, int ilw,
            unsigned char *out, int ox, int oy, int ow, int oh, int olw);
...
void SendEventEx(iv_handler hproc, int type, int par1, int par2, void* data);
void* GetCurrentEventExData(); // should be called from event handler

void ProcessEventLoop();
void ProcessEventLoopQuick();
void FlushEvents();
...
/*
 * returns CLOCK_BOOTTIME content in ms
 */
...
irect GetKeyboardRectWithParams(const char *title, char *buffer, int flags);
...
/* Copy framebuffer from task to self*/
void CopyTaskFb(int task);

int GetFrontlightVersion(void);

/*
 * Returns frontlight state (0-100 - brightness)
 * returns INT_MIN if can't read or not supported
 */
int GetFrontlightState(void);

/*
 * Sets frontlight state (0-100 - brightness)
 */
...
ibitmap *GetBookCoverEx(const char* name, int width, int height, PixelFormat pixel_format);
...
typedef enum AudioPlayerType_e {
    kAudioPlayer = 1,
    kAudioBooks = 2,
    kBrowser = 4,
    kReaderPlayer = 8,
...
typedef struct AudioPlayingInfo_s {
 ...
 */
bool isUsbStorAttached();
/*
 * Check if usb storage is mounted
 */
bool isUsbStorMounted();
/*
 * Send command for eject usb storage
 */
void usbStorEject();
/*
 * Returns usb storage serial number, allocated in dynamic memory.
 * It should be freed when has used.
 * Otherwise func returns NULL.
 */
...
void SetAutoPowerOff(bool en);
void SetAutoKeylock(bool en);
void PowerOff();
int SafeMode();
void OpenMainMenu();
void CloseAllTasks();
int WriteStartupLogo(const ibitmap *bm);
void WritePartnerPowerOffLogo(const ibitmap* bm);
...
int SetBluetoothMode(int mode, int flags); //deprecated

bt_state GetBluetoothStatus();

/*
 * Starts bluetooth manager and enable hardware and driver
 */
int SetBluetoothOn(void);

/*
 * Stops bluetooth manager and disable hardware and driver
 */
int SetBluetoothOff(void);

/*
 * Returns bluetooth manager status, means 1 - running; 0 - stopped
 */
int IsBluetoothEnabled(void);

/*
 * BluetoothWakeUp and BluetoothSuspend functions for control bluetooth power save mode.
 * To check current driver mode use func  IsBluetoothAwake
 */
int IsBluetoothAwake();
int BluetoothWakeUp();
int BluetoothSuspend();
...
/*
 * Disable wifi and bt modules but keep their managers enabled.
 */
int SetFlightMode(bool fm);
...
char *GetProxyUrl();
void *QuickDownloadExt3(const char *url, int *retsize, int timeout, char *cookie, char *post, int * error_code);
...
/*
 * Return count of touch section, which are contained in container pointed by @index
 */
unsigned int GetTouchContainerLength(unsigned int index);

/*
 * Return iv_mtinfo_section address pointed by @section in touch container with @index.
 * If no points are in container, then return NULL.
 */
iv_mtinfo_section *GetTouchSection(unsigned int index, unsigned int section);

/*
 * Return last iv_mtinfo_section address. It is location of last points, that was added to container with @index
 */
iv_mtinfo_section *GetLastTouchSection(unsigned int index);

/*
 * Return last iv_mtinfo_section address and copy to @count rest count of section in touch container pointed by @index
 * Each calling decrease internal section counter.
 * If no points are in container, then return NULL.
 */
iv_mtinfo_section *PopTouchSection(unsigned int index, unsigned int *count);
...
int utf2ucsEx(const char *s, int len, unsigned short *us, int maxlen);
...
int get_screen_dpi();
double get_screen_scale_factor();
...
// Should not be called more than once by application
int set_usleep_func(usleep_func_t f);
 ...
 * @param genre
 */
void setAvrcpMetadata(const char* path, int length, const char* title, const char* artist, const char* album, const char* genre);

/**
 * @brief get_audio_status (this function can take some time)
 * @param status
 * @return 1 - on success
 */
...
void needCanvasLock();
void lockCanvasDrawing();
void unlockCanvasDrawing();
#define BTGATT_CLIENT_PATH "/usr/bin/btgatt-client"
#define BTGATT_SERVICE_PATH "/usr/libexec/bluetooth/gatt-service"

/*
 * Nodes represent communication interface between gatt-service and app.
 * BTGATT_SERVICE_RX_NODE - file with read data buffer from client.
 * BTGATT_SERVICE_TX_NODE - file for write data buffer to client.
 * BTGATT_SERVICE_STATUS_NODE - status gatt-service. It can be "disconnected" or "connected"
 */
#define BTGATT_SERVICE_RX_NODE "/var/run/bt_gatt_services/0000ffff-0000-1000-8000-00805f9b34fb/0001"
#define BTGATT_SERVICE_TX_NODE "/var/run/bt_gatt_services/0000ffff-0000-1000-8000-00805f9b34fb/0002"
#define BTGATT_SERVICE_STATUS_NODE "/var/run/bt_gatt_services/status"

/*
 *	Run Gatt service, which is started as daemon.
 *	Use BTGATT_SERVICE_RX_NODE, BTGATT_SERVICE_TX_NODE, BTGATT_SERVICE_STATUS_NODE for communication with it.
 *	Return 0 if OK and <0 if error
 *	@name - advertisement name of device. It is shown by scan process;
 */
int StartCustomGattService(char *name);
/*
 * Stop Gatt service
 */
int StopCustomGattService(void);

enum CustomGattSessionStatus_e {
	GattSessionStatus_uncknown = 0,
	GattSessionStatus_disconnected,
	GattSessionStatus_errorCommon,
	GattSessionStatus_errorConnection,
	GattSessionStatus_errorService,
	GattSessionStatus_connecting,
	GattSessionStatus_discoveringServices,
	GattSessionStatus_connected

};

typedef struct CustomGattSession_s CustomGattSession;

/*
 * Return gatt-session status
 */
enum CustomGattSessionStatus_e GetCustomGattSessionStatus(CustomGattSession *obj);
/*
 * Create connection and transfer data session.
 * @dest_mac_addr mac address to connect in string representation, e.g. "11:22:33:44:55:66"
 * Return pointer to CustomGattSession object of NULL if error happened.
 * Returned object should be released by CloseCustomGattConnection() function after using.
 */
CustomGattSession *Connect2CustomGattService(char *dest_mac_addr);
/*
 * Use for release connection session
 */
int CloseCustomGattConnection(CustomGattSession *s);
/*
 * Read data from gatt-service by client.
 * Data will be stored to @data.
 * Data length is limited to 20 bytes + 1 '\0'(end). Data has string representation.
 * Return quantity of read symbols or <0 if error detected.
 */
int ReadCustomGattService(CustomGattSession *obj, char *data);
/*
 * Write data to gatt-service by client. Data should be in string representation.
 * @data buffer with actual @data_len.
 * Return <0 if sending was fail or error happened, otherwise return 0.
 */
int WriteCustomGattService(CustomGattSession *obj, const char *data, size_t data_len);
/*
 * Start scan bluetooth LE process.
 * @void (*cb)(void *cb_data, char *mac, char *name) callback function, which will be called if some BTLE device will be found.
 * Callback sets pointers to strings with @mac address and @name. if Error happened @cb is called with @mac=NULL, @name=NULL
 * example Callback:
 * void print_scan(void *d, char *m, char *n) {
 *	printf("mac = [%s]; name = [%s]\n",m,n);
 *	}
 * StartScanBTLE return 0 if OK; otherwise <0 if error.
 * To stop scanning process StopScanBTLE() function should be used.
 */
int StartScanBTLE(void (*cb)(void *cb_data, const char *mac, const char *name), void *cb_data);
/*
 * Stop bluetooth LE scan process.
 */
int StopScanBTLE();
