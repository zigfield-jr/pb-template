#ifndef INKINTERNAL_H
#define INKINTERNAL_H

#include "inkplatform.h"
#include "inkview.h"
#include <ft2build.h>

#include FT_FREETYPE_H
#include FT_GLYPH_H
#include FT_IMAGE_H
#include FT_OUTLINE_H

#ifndef EMULATOR
#include <dlfcn.h>
#include <sys/ioctl.h>
#include <linux/rtc.h>

#include <sys/socket.h>
#include <net/if.h>
#include <netinet/in.h>

#else
#endif

#ifdef __cplusplus
extern "C" {
#endif

//#define IVDBG(x...) fprintf(stderr, x);
#define IVDBG(x...)

#define PBDRM_PDFREADER "pdfviewer.app"

#define MAXPATHLENGTH 1024
#define MAXEVENTS 128  //must be powers of 2
#define MAXDICTS 255

#define SLEEPDELAY 1200LL                                               // delay sleep after wake-up or app start
#define EVENTDELAY 500LL                                                // delay sleep after keypress/touch event
#define EINKDELAY ((HWC_A13 || HWC_B288 || HWC_B300) ? 350LL : 1500LL)  // delay sleep after eink update

#define MAXVOL 100
#define MAXUIDATA 4096

#define MAXPANELANIMATIONTASKS 8

#define EPDC_PIXELCHANGED 0x80000000
#define EPDC_GRAYSCALE    0x40000000
#define EPDC_COLLISION    0x20000000
#define EPDC_FULL         0x10000000
#define EPDC_FORCELUT     0x08000000
#define EPDC_LOGO         0x04000000
#define EPDC_PARTIALHQ    0x02000000

#define SHOWCLOCK_ONTURN 0
#define SHOWCLOCK_NOHIDE 1
#define SHOWCLOCK_ALWAYS 2
#define SHOWCLOCK_OFF 3

enum { WAIT_NONE=0, WAIT_HARD=1, WAIT_SOFT=2 };

enum usb_state_e {
	USBSTATE_ONLINE					= 1,
	USBSTATE_PCLINK					= 2,
	USBSTATE_MASS_STORAGE_MOUNTED	= 4,
	USBSTATE_MASS_STORAGE_EJECT		= 8,
	USBSTATE_CHARGING				= 16, //deprecated...use mpc->power_state with POWER_STATE_CHARDGING;
	USBSTATE_USBNET					= 32,
	USBSTATE_MASS_STORAGE_ATTACHED 	= 64,
	USBSTATE_SND					= 128, //usb sound card
};

#define PWRACT_LAST 0
#define PWRACT_LOCK 1
#define PWRACT_MMNU 2
#define PWRACT_EXIT 3
#define PWRACT_SCRN 4
#define PWRACT_PROF 5
#define PWRACT_SPWR 6
#define PWRACT_MP3O 7
#define PWRACT_BTSW 8
#define PWRACT_TASK 9
#define PWRACT_DRPC 29
#define PWRACT_STTY 30
#define PWRACT_NONE 31

#define IN_LASTOPEN 2
#define IN_DIALOG 4
#define IN_MENU 8
#define IN_KEYBOARD 16
#define NO_WAIT_UPDATE_COMPLETE 32
#define NO_HOURGLASS 64
#define NO_DISPLAY 128
#define IS_EXTERNAL 512
#define COVER_FRAME 1024

#define UIQ_NONE           0
#define UIQ_MESSAGE        1
#define UIQ_DIALOG         2
#define UIQ_EVENT          3
#define UIQ_TEXTENTRY      4
#define UIQ_PROGRESSBAR    5
#define UIQ_NPROGRESSBAR   6
#define UIQ_UPDATE         7
#define UIQ_EVENT_MAIN     8
#define UIQ_OPENBOOK       9
#define UIQ_DISMISS      127

#define UIQ_IDLE           0
#define UIQ_PENDING        1
#define UIQ_PROCESSING     2
#define UIQ_OK             3
#define UIQ_CANCEL         4

#define UPD_RESULT_NONE        0
#define UPD_RESULT_QUERY       5
#define UPD_RESULT_FAIL       10
#define UPD_RESULT_CANCELED   11
#define UPD_RESULT_NOIMAGE    12
#define UPD_RESULT_CORRUPTED  13
#define UPD_RESULT_POWERLOW   14
#define UPD_RESULT_WRONGMODEL 15
#define UPD_RESULT_OK         99

#define RECT_ROUND_TL	(1 << 0)
#define RECT_ROUND_TR	(1 << 1)
#define RECT_ROUND_LL	(1 << 2)
#define RECT_ROUND_LR	(1 << 3)

#define INKFLAG_SW_16GRAYS     0x01
#define INKFLAG_3BIT_WAVEFORM  0x02
#define INKFLAG_A2PLUS         0x04

#define MCU_CAPS_NOTINITED  0x80000000
#define MCU_CAPS_FRONTLIGHT (1 << 0)
#define MCU_CAPS_SENSOR     (1 << 1)

#define IOC_MCU_PATH "/sys/class/ioc-stm32/"
#define IOC_MCU_SENSOR_PATH IOC_MCU_PATH "sensor"
#define IOC_MCU_FPROFILE_PATH IOC_MCU_PATH "frontlight_profile"
#define IOC_MCU_BRIGHTNESS_PATH IOC_MCU_PATH "brightness"
#define IOC_MCU_POWEROFF_PATH IOC_MCU_PATH "poweroff"
#define IOC_MCU_RESET_PATH IOC_MCU_PATH "reset"

#define POWER_STATE_CHARDGING	(1 << 0)
#define POWER_STATE_FULL		(1 << 1)

typedef enum EPARTNER_ID {
    PARTNER_INVALID = -1,
    PARTNER_DEFAULT = 0,
    PARTNER_TITELIVE,
    PARTNER_OSIANDER,
    PARTNER_MAYERSCHE,
    PARTNER_IBS,
    PARTNER_LEGALO,
    PARTNER_TEA,
    PARTNER_ALZA,
    PARTNER_EBOOKIT,
    PARTNER_EMPIK,
    PARTNER_APOLLO,
    PARTNER_MENCA_,
    PARTNER_FONA,
    PARTNER_LIBRONET,
    PARTNER_UMBREIT,
    PARTNER_KNV,
    PARTNER_NORLI,
    PARTNER_BUCHMEDIA,
    PARTNER_LEGIMI,
    PARTNER_BUCHZENTRUM,
    PARTNER_MINOFDEF,
    PARTNER_VIVLIO,
    PARTNER_BOEKHANDEL,
    PARTNER_CUSTOM,
    PARTNER_EMPIKGO,
    PARTNER_EXLIBRIS
} PARTNER_ID;

//#define DEEPSLEEPTIME 600000LL
#define DEEPSLEEPTIME 300000LL
#define BATTERYCHECKTIME 7200000LL
#define MINSUSPENDTIME (5000LL)

#define A13_SLEEP_REFRESH_TIME 300000LL // 5 minutes

#define DEVICEID_LEN        32    

#define SN_ORSIO "92457"

#define C24TO8(c) (((((c) >> 16) & 255) * 77 + (((c) >> 8) & 255) * 151 + ((c) & 255) * 28) >> 8)

#define ISLTRALPHA(c)  (((c) >= 0x41 && (c) <= 0x5a)     || \
            ((c) >= 0x61 && (c) <= 0x7a)     || \
            ((c) >= 0xc0 && (c) <= 0x2b8)    || \
            ((c) >= 0x386 && (c) <= 0x587))

#define ISRTLALPHA(c)  (((c) >= 0x591 && (c) <= 0x6ff)   || \
            ((c) >= 0xfb1d && (c) <= 0xfdff) || \
            ((c) >= 0xfe70 && (c) <= 0xfefc))

#define ISRTLPUNCT(c)  (((c) >= 0x20 && (c) <= 0x2f)     || \
            ((c) >= 0x3a && (c) <= 0x3f)     || \
            ((c) >= 0xa1 && (c) <= 0xbf)     || \
            ((c) >= 0x2010 && (c) <= 0x2046))

#define ISRTLDIGIT(c) (((c) >= '0' && (c) <= '9') || ((c) == '%'))

#define ISBRACKETS(c) ((c) == '(' || (c) == ')' || (c) == '[' || (c) == ']' || (c) == '{' || (c) == '}' || (c) == '<' || (c) == '>')

#define DEFAULT_3G_CONNECTION_NAME "@Default_3G_conn"

#if ! (defined(EMULATOR) || defined(IVSAPP))
#define MULTITASKING (ivmpc->vid == 0x544d)
#define ISACTIVETASK ((!MULTITASKING) || ivmpc->activetask == ivstate.taskid)
#else
#define MULTITASKING 0
#define ISACTIVETASK 1
#endif

#define TASKMQADDR(t) (0x7a000000 | (t))

typedef struct iv_netcookie_s {

    int enumid;
    char *name;
    char *value;
    char *domain;
    char *path;
    int name_len;
    int value_len;
    int domain_len;
    int path_len;

} iv_netcookie;

typedef struct iv_scroll_s {
    int window_x;
    int window_y;
    int window_w;
    int window_h;
    int content_x;
    int content_y;
    int content_h;
    int content_w;
//	int current_pos;
//	int count_elements;
//	int count_elements_on_page;
//	int percet_present_element;
} scroll_s;

typedef struct iv_glyph_s {

    unsigned short chr;
    short left;
    short top;
    unsigned char width;
    unsigned char height;
    unsigned char data[0];

} iv_glyph;

typedef struct iv_state_s {

    int isopen;
    const char *theme;
    const char *lang;
    const char *kbdlang;
    unsigned char pwract1;
    unsigned char pwract2;
    unsigned char timeformat;
    unsigned char gsense;
    unsigned char showclock;
    unsigned char _reserved1;
    unsigned char keylock;
    unsigned char demo;
    unsigned char antialiasing;
    unsigned char uiquery;
        unsigned char canbedefaultfont;
    unsigned char _reserved3;

    pid_t pid;
    iv_handler hproc;
    iv_handler mainhproc;

    icanvas *framebuffer;
    icanvas *canvas;
    int orientation;  // 0, 1, 3, 2

    iconfig *cfg;
    FT_Library ftlibrary;
    ihash *fonthash;
    const ifont *current_font;
    FT_Face current_face;
    int current_color;
    int current_aa;
    unsigned char **cw_cache;
    unsigned short **kerning_cache;
    iv_glyph **glyph_cache;

    const char *font;
    const char *fontb;
    const char *fonti;
    const char *fontbi;

    int keylocktm;
    int powerofftm;
    int allowkeyup;
    int currentkey;
    int keyrepeats;
    int initialized;
    volatile sig_atomic_t exiting;
    int usbmode;
    int uiflags;
    int taskid;
    int needupdate;
    bool poweroffenable;
    bool keylockenable;
    int sleepenable;
    long long waketime;
    long long sleeptime;
    long long measuretime;

    itimer *timers;
    int ntimers;

    ievent *events;
    int evtstart;
    int evtend;
    unsigned int awaiting_pmove_events;

    int keytime1;
    int keytime2;
    int msgtop;

    int ptrx;
    int ptry;

    ifont **fontlist;
    int fontcount;

    struct {
        int raw;
        int a;
        int b;
        int c;
        int d;
        int e;
        int f;
        int nx;
        int ny;
    } tsc[MAX_INPUT_TOUCH_DEV];

    FT_Face current_zface;
    unsigned int touchdragdeadzone;
    int panel_position;  //0 - bottom, 1  - top
    time_t adjtime_ts;
    char *locale;
    int audio_app_type; // 0 - nothing, 1 - audio player, 2 - audio book, 3 - browser, 4 - reader player
    int capability;
} iv_state;

typedef struct iv_caches_s {

    int size;
    int refs;
    unsigned char **cw_cache;
    unsigned short **kerning_cache;
    iv_glyph **glyph_cache_aa;
    iv_glyph **glyph_cache_noaa;

} iv_caches;

typedef struct iv_fontdata_s {

    int refs;
    FT_Face face;
    FT_Face zface;
    int nglyphs;
    int nsizes;
    iv_caches *caches;

} iv_fontdata;

typedef struct iv_uiqrecord_s {

    int type;
    int icon;
    char *title;
    char *text;
    char *button1;
    char *button2;
    int maxsize;
    int flags;
    int timeout;
    int percent;
    int evtype;
    int evpar1;
    int evpar2;
    int reserved_01;
    int reserved_02;
    int reserved_03;
    int reserved_04;
    int reserved_05;

} iv_uiqrecord;

typedef struct iv_event_s {

    int type;
    int mod;
    int par1;
    int par2;

} iv_event;

/*
 * EVT_EVENTS ver.II
 * The first bit of event field is determined that using event type II
 */

typedef struct hw_event_data_common_s {
	int mod;
	int par1;
	int par2;
} hw_event_data_common;

//EVT_POINTERUP, EVT_POINTERDOWN, EVT_POINTERMOVE
typedef struct hw_event_data_pointer_s {
	int id;
	int x;
	int y;
	int pressure;
    int tilt_x;
    int tilt_y;
	enum input_dev_e devtype;
} hw_event_data_pointer;

typedef struct hw_event_s {
	int type;
	union {
		hw_event_data_common common;
		hw_event_data_pointer pointer;
	};
    struct timeval time;
} hw_event;

typedef enum fl_mode_e {
	FL_MODE_AUTO_BRIGHTNESS = 1,
	FL_MODE_AUTO_COLOR = 2,
    FL_MODE_ENABLED = 0x80 // if true frontlight turned on
} fl_mode_t;

typedef struct hw_frontlight_s {
	int version;

	/*
     *  MANUAL if mode == 0 or check bitfields fl_mode_t
	 */
    unsigned char mode;
	int color;
	int brightness;
} hw_frontlight;

enum gyro_mode_e {
	GYRO_MODE_ENABLED = 0x01,
	GYRO_MODE_INT = 0x02, //if this mode is enabled, then monitor sends EVT_ORIENTATION for all task
};

typedef struct hw_gsensor_s {
	int version;

	/*
	 * bitfields of enum gyro_mode_e
	 */
	uint8_t mode;
	int8_t orientation;
} hw_gsensor;

typedef enum audio_redirector_state_e {
    AUDIO_REDIRECTOR_NOT_RUNNING,
    AUDIO_REDIRECTOR_RUNNIG,
    AUDIO_REDIRECTOR_NO_OUTPUT,
    AUDIO_REDIRECTOR_CONFIGURING,
} audio_redirector_state_t;

typedef struct avrcp_props_s {
    int playback_status; // 0 - stopped, 1 - playing, 2 paused
    int volume;             // 0 to 100
    int position;           // in sec
    int length;             // in sec
    int track_id;
} avrcp_props_t;

typedef enum { // 0 - unknown; 1 - checked, no errors; 2 - checked, with errors
    FS_STATE_UNKNOWN = 0,
    FS_STATE_CHECKED_OK,
    FS_STATE_CHECKED_ERR,
    FS_STATE_CHECKED_FAIL
} FS_STATE_INT;

typedef struct iv_mpctl_s {

    unsigned char reserved0;
    unsigned char poweroff;
    unsigned char lockdisplay;
    unsigned char capstate;
    unsigned char sdstate;
    unsigned char wakeup;
    unsigned char safe_mode;
    unsigned char inkflags;
    unsigned char orientation;
    volatile unsigned char keylock;
    enum usb_state_e usbstate;
    unsigned char umoverride;

    pid_t watchpid;
    pid_t mainpid;
    pid_t apppid;
    pid_t mppid;
    pid_t nagtpid;
    int activetask;
    int activesubtask;

    int power;
    int power_state;

    int pkey;
    int vid;

    PB_STATE player_state;
    short mode;
    short track;
    PB_TTS_STATE tts_state;
    short scanning_state;
    short adobe_scanning_state;
    short text_recognition_flag;
    short reserved_9;

    int tracksize;
    int position;
    int newposition;

    short volume;
    short equalizer[32];
    short filter_changed;
    short reserved_0c;

    unsigned char btready; // status of bt driver 0 = unloaded (can sleep), 1 = loaded (can sleep only if true (bt_flags & BT_FLAG_LOW_POWER))
    unsigned char wifiready;
    unsigned char cdma3gready;
    unsigned char connected;
    unsigned char rfcomm;
    unsigned char netscan;
    unsigned char flightmode;
    unsigned char reserved_0e;
    unsigned char reserved_0f;

    char netname[64];
    char netdevice[16];
    char netsecurity[16];
    char netprefix[16];
    int con_index;
    int con_time;
    int con_speed;
    int net_signal_quality;

    NET_STATE netstate;
    int last_connection_error;

    unsigned long bytes_in;
    unsigned long bytes_out;
    unsigned long packets_in;
    unsigned long packets_out;
    unsigned long net_timeout;

    //bluetooth
    pid_t nagtpid_bt;
    bt_mode bt_mode;
    bt_state bt_status; //enum bt_state_e
    unsigned int  bt_flags; // 1 - update timeout
    unsigned long reserved_11;
    unsigned long reserved_12;

    audio_output_t audio_output;

    unsigned long keymask;
    time_t lastactivity;
    unsigned long gka0; //keymask for global short press action
    unsigned long gka1; //keymask for global long press action
    unsigned long gka2; //keymask for global double click action
    int uisequence;
    int uiquery;
    int uistatus;
    unsigned int uidata[MAXUIDATA];
	int obreey_sync_status;
	int messages_count; // Number of new messages to show on panel
	int dialog_h[16];   // heights of dialog windows
	int headphone_status;
	int frontlight_state;
	int frontlight_slider;
	int books_db_changes;
	time_t adjtime_mono;
    int unsafe_volume_timer;
    int lux_auto;
    int lux_bright[101];
    int frontlight_auto;

	struct {
		unsigned int numframes;
		int fps;
		int x;
		int y;
	} hourglass;

	pid_t bstask;
	pid_t exptask;
	pid_t mgrtask;
	pid_t scannertask;
	pid_t obreeypid;
	pid_t obreey_social_pid;
	pid_t usage_stat_pid;
	pid_t dropbox_pid;
	pid_t demoviewerpid;
	pid_t send_to_pb_pid;
	pid_t control_panel_pid;
	pid_t menca_pid;
    pid_t push_notification_pid;
    pid_t reader_controller_pid;
    pid_t db_server_pid;

    int enable_timed_refresh;
    unsigned char user_activity;
    unsigned char reserved_14;
    unsigned char reserved_15;
    unsigned char reserved_16;

    int panel_animation_phase;
    long long panel_animation_auto_stop[MAXPANELANIMATIONTASKS];
    int panel_animation_auto_pid[MAXPANELANIMATIONTASKS];
    long long obex_transferring_id;
    long long obex_transferred;
    long long obex_size;
    int menca_status;
    int partner_change_counter;
    pid_t netscript_start_pid;
    time_t high_priority_job_valid_until[2]; // sec

    //frontlight ver 2
    hw_frontlight fl;

    //gyroscope interface
    hw_gsensor gyro;
    long long fake_bootclock_sleep_time_ms;

    // audio
    AudioPlayerType audio_player_type;
    audio_redirector_state_t audio_redirector_state;
    avrcp_props_t avrcp_props;
    int do_not_show_bt_configurator;
    int configure_audio_output_async_running;

    PB_STATE audio_book_state;
    int avrcp_receiver_pid;
    PB_STATE browser_state;
    pid_t browser_playing_pid;

    int package_processing_progress;
    int package_processing_progress_pid;

    struct {
        unsigned char ext1;  // 0 - unknown; 1 - checked, no errors; 2 - checked, with errors
        unsigned char ext2;
        unsigned char ext3;
    } filesystem_state; // FS_STATE_INT

    PB_STATE reader_player_state;
    int screen_mode_invert;

    // screen reader related flags
    bool screen_reader_enabled;
    bool screen_reader_daemon_operational;
    bool input_blocked_by_screen_reader;
    bool screen_reader_is_playing;
    int text_scale_percentage;
    int text_bold_enabled;
    int text_contrast_enabled;
    int text_dyslexic_font_enabled;
} iv_mpctl;

typedef struct eink_cmd_s {

    int owner;
    int command;
    int nwrite;
    int nread;
    unsigned char data[0];

} eink_cmd;

typedef struct eink_updateinfo_s {
    int x;
    int y;
    int w;
    int h;
    int bw;
} eink_updateinfo;

typedef struct upd_status_s {

    unsigned long action_timestamp;
    char version[16];
    unsigned long sw_timestamp;
    unsigned long sw_info1;
    unsigned long sw_info2;
    unsigned long sw_info3;
    unsigned char source;
    unsigned char unused;
    unsigned char result;
    unsigned char commit;

} upd_status_record;

struct id2code_s {
    unsigned idx;
    unsigned code;
};

struct iv_img_conv {
    const void *src;
    void *dest;
    int width;
    int height;
    int src_scanline;
    int dest_scanline;
};
 
typedef struct cookie_node_s {
    iv_netcookie * cookie;
    struct cookie_node_s * next;
} cookie_node;

extern iv_mtinfo mtinfo[5];
extern iv_state ivstate;
extern iv_mpctl *ivmpc;
extern iv_fbinfo *fbinfo;

extern ifont *title_font, *window_font, *header_font, *menu_s_font, *menu_n_font;
extern ifont *butt_n_font, *butt_s_font;
extern int window_color, header_color, menu_n_color, menu_s_color, menu_d_color;
extern int butt_n_color, butt_s_color;
extern ibitmap *button_normal, *button_selected;
extern ibitmap *button_back, *button_prev, *button_next;
extern int title_style, title_height;
extern iapp_caption *app_caption;
extern itaskmgr *taskmgr;

#define NO_INTERNAL_IMAGES
#ifndef NO_INTERNAL_IMAGES
extern ibitmap def_fileicon, def_folder;
extern ibitmap def_hourglass, def_lock;
extern ibitmap def_button_normal, def_button_selected;
extern ibitmap def_pageentry;
extern ibitmap def_bmk_panel, def_bmk_active, def_bmk_inactive, def_bmk_actnew;
extern ibitmap def_tab_panel, def_tab_active, def_tab_inactive;
extern ibitmap def_leaf_open, def_leaf_closed;
extern ibitmap def_battery_0, def_battery_1, def_battery_2, def_battery_3, def_battery_4;
extern ibitmap def_battery_5, def_battery_c, def_battery_u;
extern ibitmap def_button_back, def_button_prev, def_button_next;
extern ibitmap def_progress_left, def_progress_right, def_progress_empty, def_progress_filled;
extern ibitmap def_tsk_arrow, def_tsk_arrow_a, def_tsk_clr, def_tsk_clr_a, def_tsk_bs, def_tsk_bs_a;
extern ibitmap def_tsk_enter, def_tsk_enter_a, def_tsk_can, def_tsk_can_a, def_tsk_123, def_tsk_123_a;
extern ibitmap keyboard_arrow, keyboard_bs, keyboard_eye;
extern ibitmap keyboard_word_left, keyboard_word_center, keyboard_word_right, keyboard_marker;
extern ibitmap keyboard_close_bg, keyboard_close_button, keyboard_area_filler, keyboard_foot_center;
extern ibitmap keyboard_button_center, keyboard_button_center_a, keyboard_button_sep;
extern ibitmap keyboard_search, keyboard_comment;
extern ibitmap def_qi_btsw, def_qi_lock, def_qi_mmnu, def_qi_mp3o, def_qi_scrn, def_qi_spwr;
extern ibitmap def_p_book, def_p_loading;
extern ibitmap def_playing, def_usbinfo, def_gprsinfo, def_wifiinfo, def_cdma3ginfo, def_usbex;
extern ibitmap def_bluetooth, def_a2dpheadset, def_tasklist;
extern ibitmap def_frame_selector_bl_5px, def_frame_selector_br_5px;
extern ibitmap def_frame_topleft, def_frame_top, def_frame_topright, def_header_topleft, def_header_top;
extern ibitmap def_header_topleft_overlap, def_header_topright_overlap;
extern ibitmap def_frame_topleft_overlap, def_frame_topright_overlap;
extern ibitmap def_header_topright, def_frame_left, def_frame_right, def_frame_bottomleft;
extern ibitmap def_frame_bottom, def_frame_bottomright;
extern ibitmap def_button_moveup, def_button_movedown, def_button_close;
extern ibitmap def_button_moveup_a, def_button_movedown_a, def_button_close_a;
extern ibitmap def_dic_toolbar, def_dic_wordlist, def_dic_voice, def_dic_keybd, def_dic_switch;
extern ibitmap def_dic_dots, def_dic_wordbg, def_dic_error, def_dic_close, def_dic_area, def_dic_hdivider;
extern ibitmap def_dic_translate;
extern ibitmap swupdate_bm;
extern ibitmap obreey_sync_wrong_settings, obreey_sync_error, obreey_sync_idle, obreey_sync_downloading,
       obreey_sync_paused;
#else
extern ibitmap def_broken_image;
#define def_fileicon def_broken_image
#define def_folder def_broken_image
#define def_hourglass def_broken_image
#define def_lock def_broken_image
#define def_a2dpheadset def_broken_image
#define def_battery_0 def_broken_image
#define def_battery_1 def_broken_image
#define def_battery_2 def_broken_image
#define def_battery_3 def_broken_image
#define def_battery_4 def_broken_image
#define def_battery_5 def_broken_image
#define def_battery_c def_broken_image
#define def_battery_u def_broken_image
#define def_bluetooth def_broken_image
#define def_button_back def_broken_image
#define def_button_close_a def_broken_image
#define def_button_close def_broken_image
#define def_button_movedown_a def_broken_image
#define def_button_movedown def_broken_image
#define def_button_moveup_a def_broken_image
#define def_button_moveup def_broken_image
#define def_button_next def_broken_image
#define def_button_prev def_broken_image
#define def_calc_button def_broken_image
#define def_cdma3ginfo def_broken_image
#define def_dic_area def_broken_image
#define def_dic_close def_broken_image
#define def_dic_dots def_broken_image
#define def_dic_error def_broken_image
#define def_dic_hdivider def_broken_image
#define def_dic_keybd def_broken_image
#define def_dic_switch def_broken_image
#define def_dic_toolbar def_broken_image
#define def_dic_translate def_broken_image
#define def_dic_voice def_broken_image
#define def_dic_wordbg def_broken_image
#define def_dic_wordlist def_broken_image
#define def_frame_bottom def_broken_image
#define def_frame_bottomleft def_broken_image
#define def_frame_bottomright def_broken_image
#define def_frame_left def_broken_image
#define def_frame_right def_broken_image
#define def_frame_selector_bl_5px def_broken_image
#define def_frame_selector_br_5px def_broken_image
#define def_frame_top def_broken_image
#define def_frame_topleft def_broken_image
#define def_frame_topleft_overlap def_broken_image
#define def_frame_topright def_broken_image
#define def_frame_topright_overlap def_broken_image
#define def_gprsinfo def_broken_image
#define def_header_top def_broken_image
#define def_header_topleft def_broken_image
#define def_header_topleft_overlap def_broken_image
#define def_header_topright def_broken_image
#define def_header_topright_overlap def_broken_image
#define def_leaf_closed def_broken_image
#define def_leaf_open def_broken_image
#define def_p_book def_broken_image
#define def_playing def_broken_image
#define def_p_loading def_broken_image
#define def_progress_empty def_broken_image
#define def_progress_filled def_broken_image
#define def_progress_left def_broken_image
#define def_progress_right def_broken_image
#define def_qi_btsw def_broken_image
#define def_qi_lock def_broken_image
#define def_qi_mmnu def_broken_image
#define def_qi_mp3o def_broken_image
#define def_qi_scrn def_broken_image
#define def_qi_spwr def_broken_image
#define def_tasklist def_broken_image
#define def_tsk_123_a def_broken_image
#define def_tsk_123 def_broken_image
#define def_tsk_arrow_a def_broken_image
#define def_tsk_arrow def_broken_image
#define def_tsk_bs_a def_broken_image
#define def_tsk_bs def_broken_image
#define def_tsk_can_a def_broken_image
#define def_tsk_can def_broken_image
#define def_tsk_clr_a def_broken_image
#define def_tsk_clr def_broken_image
#define def_tsk_enter_a def_broken_image
#define def_tsk_enter def_broken_image
#define def_usbex def_broken_image
#define def_usbinfo def_broken_image
#define def_wifiinfo def_broken_image
#define dot_image def_broken_image
#define keyboard_area_filler def_broken_image
#define keyboard_arrow def_broken_image
#define keyboard_bs def_broken_image
#define keyboard_button_center_a def_broken_image
#define keyboard_button_center def_broken_image
#define keyboard_button_sep def_broken_image
#define keyboard_close_bg def_broken_image
#define keyboard_close_button_a def_broken_image
#define keyboard_close_button def_broken_image
#define keyboard_comment def_broken_image
#define keyboard_eye def_broken_image
#define keyboard_foot_center def_broken_image
#define keyboard_marker def_broken_image
#define keyboard_search def_broken_image
#define keyboard_word_center def_broken_image
#define keyboard_word_left def_broken_image
#define keyboard_word_right def_broken_image
#define menu_11 def_broken_image
#define menu_13 def_broken_image
#define menu_15 def_broken_image
#define new_ava_0 def_broken_image
#define new_ava_1 def_broken_image
#define new_ava_2 def_broken_image
#define new_ava_3 def_broken_image
#define new_ava_4 def_broken_image
#define new_ava_5 def_broken_image
#define new_ava_6 def_broken_image
#define new_ava_7 def_broken_image
#define obreey_sync_downloading def_broken_image
#define obreey_sync_error def_broken_image
#define obreey_sync_idle def_broken_image
#define obreey_sync_paused def_broken_image
#define obreey_sync_wrong_settings def_broken_image
#define ok_11 def_broken_image
#define ok_13 def_broken_image
#define ok_15 def_broken_image
#define ok_7 def_broken_image
#define ok_9 def_broken_image
#define pb_left def_broken_image
#define pb_left_empty def_broken_image
#define pb_progress def_broken_image
#define pb_right def_broken_image
#define pb_right_empty def_broken_image
#define qnarrow def_broken_image
#define selarrows def_broken_image
#define swupdate_bm def_broken_image
#define toolerasebig def_broken_image
#define toolerasesmall def_broken_image
#define toolexit def_broken_image
#define toolmove def_broken_image
#define toolpenbig def_broken_image
#define toolpenmedium def_broken_image
#define toolpensmall def_broken_image
#define toolsaveexit def_broken_image
#define toolundo def_broken_image

#endif

int hw_init();
bool hw_is_initialized();

int hw_io_init();
int hw_eink_init();
int hw_gsensor_init();
int hw_touchpanel_init();
bool hw_is_touch_enabled(void);
void hw_touch_enable(bool onOff);

void hw_close();
void hw_io_close();
void hw_eink_close();
void hw_gsensor_close();
void hw_touchpanel_close();

int hw_safemode();
int hw_einkversion();
void hw_displaysize(int *w, int *h);
void hw_set_fb_orientation(icanvas *fb, int n);
int hw_update_fb_orientation(icanvas *fb);
int hw_ioctl(int cmd, void *data);
icanvas *hw_getframebuffer();
int hw_update(int x, int y, int w, int h, int flags);
int hw_is_update_pending();
void hw_fullupdate();
void hw_fullupdate_hq();
void hw_get_wftimes(uint16_t result[16]);
void hw_switch_dynamic(int n);
void hw_override_update(int mode);
int hw_suspend_display();
void hw_resume_display();
void hw_restore_screen(int deep);
void hw_wait_luts_complete(int mode);
int hw_useraction();
int hw_keystate(int key);
int hw_init_evqueue();
int hw_wait_event();
int hw_nextevent(hw_event *);
int hw_ts_active();
int hw_is_player_playing();
int hw_is_audio_book_playing();
int hw_is_browser_playing();
int hw_is_reader_player_playing();
int hw_is_screen_playing();
int hw_isttsplaying();
int hw_isscanning();
int hw_isadobescanning();
int hw_ischarging();
int hw_isusbconnected();
bool hw_isusblinked();
int hw_isusbhostconnected();
int hw_isUsbStorAttached();
int hw_isUsbStorMounted();
bool hw_isUsbSndAttached(void);
char *hw_usbStorSerialNumber(void);
int hw_issdinserted();
const char *hw_external_card_serial_number(void);
void hw_usbhostpower(int n);
int hw_power();
int hw_temperature();
int hw_sleep(int ms, int deep);
void hw_ban_suspend(int sec);
time_t hw_gettime();
long long hw_timeinms();
time_t hw_boottime(void);
long long hw_boottime_ms(void);
void hw_setalarm(int ms);
void hw_say_poweroff();
int hw_shutting_down();
void hw_registerapp(pid_t pid);
int hw_read_file(const char *name, char *buf, int size);
int hw_write_file(const char *name, const char *buf, int size);
char *hw_serialnumber();
char *hw_password();
unsigned int hw_pkey();
int hw_get_device_id(char *buffer, size_t buflen);
char *hw_waveform_filename();
char *hw_getmodel();
char *hw_getplatform();
char *hw_getmac();
char *hw_getbtmac();
char *hw_get3gimei();
int hw_geta2dpstatus(void);

typedef enum {
    OutputLogoModeFillScreen = 0,
    OutputLogoModeAdjustWithWhiteBackground = 1, // default value
    OutputLogoModeAdjustWithBlackBackground = 2,
    OutputLogoModeDepthEffect = 3,
} ivOutputLogoMode;

ibitmap *iv_create_adjusted_cover_logo(const ibitmap *bm, int is_boot_logo);

int hw_writelogo(const ibitmap *bm, int permanent);
int hw_show_poweroff_logo();
int hw_write_lowpower_logo(const char *low_power_logo);
void hw_change_logo_by_lang(int bits_logo_type); // 1 - boot logo; 2 - low battery logo
int hw_restorelogo();

int hw_usbready();
void hw_usblink();
void hw_usbStorEject();
void hw_set_keylock(int en);
int hw_get_keylock();
void hw_setglobalkeymask(unsigned long gka0, unsigned long gka1, unsigned long gka2);
int hw_touchpanel_ready();
int hw_gsensor_ready();
int hw_read_gsensor(int *x, int *y, int *z, int raw);
int hw_last_orientation();
void hw_set_gsensor(int mode);
void hw_suspend_gsensor();
void hw_resume_gsensor();
unsigned char *hw_calibrate_gsensor();
void hw_adjust_gsensor(unsigned char *data);
int hw_gsensor_orientation();

int hw_gsensor_switch(bool onOff);
int hw_gsensor_get_switch(void);

int hw_read_captouch();
int hw_sd_mounted();
int hw_flash_mounted();
int has_secure_storage();
void hw_winmessage(char *title, char *text, int flags);
int iv_check_pointerscroll(int type, int par1, int par2);

long hw_ipcrequest_to(int mq, long type, long param, void *data, int inlen, int outlen, int timeout);
long hw_ipcrequest(long type, long param, void *data, int inlen, int outlen, int timeout);
long hw_ipcrequest_secure(long type, long param, void *data, int inlen, int outlen, int timeout);
long hw_ipcreply(long addr, long type, long param, void *data, int len);
int  hw_ipcread(int addr, int *replyaddr, int *type, int *param, void *data, int size);


//==============
int hw_wifi_scan_process_start(void);
void hw_wifi_scan_process_set_selective_list(iv_wifi_ap_list *);
iv_wifi_ap_list *hw_wifi_scan_process_get_results(void);
int hw_wifi_scan_process_stop(void);
//==============
int hw_check_net_dev_working(void);
iv_wifi_ap_list * hw_wifi_scan(iv_wifi_ap_list * ap_list);

int hw_querynetwork();
int hw_btcontrol(int mode, int flags);
int hw_btwakeup ();

//check bt module status
bt_state hw_btstatus();
int hw_btupdate_timeout(void);
int is_bt_up();
int is_bt_busy();

//check wifi module status (on is 1; off is 0)
int hw_wifi_status();

//check cdmi3g module status (on is 1; off is 0)
int hw_cdma3g_status();

int hw_hidstatus(void);
char **hw_enum_btdevices();
char **hw_enum_wireless();
int hw_get_btservice(const char *mac, const char *service);
int hw_net_connect(const char *name, int silent, int showHourGlass); // silent to suppress screen output
int hw_net_disconnect();
iv_netinfo *hw_net_info();
NET_STATE hw_get_current_network_state (void);

/*
 * function creates audio config which replaces hw PCM to bluetooth PCM
 * returns 0 on success or errno if it fails
 */
int hw_create_bluealsa_config (const char *mac);

const char* get_connected_bluetooth_mac();

/*
 * function removes bluetooth PCM config and restore default audio output
 */
int hw_remove_bluealsa_config (void);

int hw_netmgr(int status);
/*
 * Returns netmgr pid.
 */
int hw_netmgr_status();
/*
 * Returns list of currently known networks in old network config format.
 * @path saving config path.
 */
int hw_netlist(const char *path);
/*
 * Adds network configured in old network config format.
 * ssid might be preset in string like \"name\" or hex format
 * key might be preset in string like \"key\" or wpa_passphrase format
 */
int hw_netadd(const char *path);
int hw_netdel(const char *path);
int hw_netdel_by_ssid(const char *ssid);
/*
 * Selects to connect already known network.
 */
int hw_netsel(const char *path);
/*
 * Selects to connect already known network.
 */
int hw_netsel_by_ssid(const char *ssid);

void hw_uiclear();
int hw_uiquery();
void hw_uiresponse(int status, char *data);

void hw_mp_setstate_ex(int state, int app_type);
void hw_mp_setstate(int state);
int hw_mp_getstate();
void hw_mp_setvolume(int n);
int hw_mp_getvolume();

void *hw_task_checkmessages(int *replyaddr, int *type, int *param, int *size);
int hw_task_register(const char *appname, const char *name, const ibitmap *icon, unsigned int flags);
int hw_task_newsubtask(char *name);
int hw_task_switchsubtask(int subtask);
void hw_task_subtaskfinished(int subtask);
int hw_task_current();
int hw_task_list(int *list, int size);
int hw_task_previous(int *task, int *subtask);
int hw_task_previous_instack(int *task, int *subtask);
taskinfo *hw_task_info(int task);
int hw_task_findbybook(char *name, int *task, int *subtask);
int hw_task_findbyappname(char *name);
int hw_task_setparameters(int task, const char *appname, const char *name, const ibitmap *icon, unsigned int flags);
int hw_task_setsubtaskinfo(int task, int subtask, const char *name, const char *book);
int hw_task_foreground(int task, int subtask);
int hw_task_foreground_with_timeout(int task, int subtask, int timeout);

void hw_task_background();
void hw_task_close(int task, int subtask, int force);
int hw_task_sendevent(int task, int hdest, int type, int par1, int par2);
int hw_task_sendevent_sync(int task, int hdest, int type, int par1, int par2);
int hw_task_message(int task, void *message, int len);
int hw_task_sendrequest(int task, int req, void *data, int inlen, int outlen, int timeout, int nowait);
int hw_task_setrequestlistener(int req, int flags);
icanvas *hw_gettaskfb(taskinfo *ti);
iv_fbinfo *hw_gettaskfbinfo(taskinfo *ti);
void hw_releasetaskfb(icanvas *fb);

/*
 * Sends msg to monitor for start and register new app with binary @path.
 * Registration params the same as used by hw_task_register function.
 * If @flags contains TASK_SELFREGISTER flag, that monitor launches app without registration.
 * It can be registered by self calling InitInkview(). Any registering params are rejected: e.g. appname, icon, flags
 * @args arguments array for execv() in the same data structure.
 * @secure select new app rights: 0 - reader; 1 - sreader; -1 - auto selection by suid.
 * Returns gpid of created process, or -1 if error happened.
 */
int hw_launchapp(const char *path, char * const args[],
		const char *appname, const char *name, const ibitmap *icon, unsigned int flags, int secure);

/* Auto control for frontlight brigthness */
// 0 - no auto frontlight; 1 - light sensor; 2 - frontlight value by time
int hw_is_frontlight_auto_supported(void);
void hw_set_frontlight_auto_enabled(int enable);
int hw_is_frontlight_auto(void);
int hw_is_frontlight_color_supported(void);
void hw_set_frontlight_color_auto_enabled(int enable);
int hw_is_frontlight_color_auto(void);
int hw_get_lux_raw(void);

int hw_get_frontlight_state(void);
void hw_set_frontlight_state(int flstate, int previous);

int hw_get_frontlight_enabled(void);
void hw_set_frontlight_enabled(int enabled);

// color set from 0 to 100
int hw_get_frontlight_color(void);
void hw_set_frontlight_color(int color);

void hw_set_frontline_color(int brightness, int color);

unsigned int hw_get_mcu_capabilities(void);
int hw_is_timed_refresh_enabled(void);
void hw_mcu_go_standby(void);
void hw_mcu_reset(void);

#define iv_extend_wtime(t) _iv_extend_wtime(__FILE__, __LINE__, t)
void _iv_extend_wtime(const char *module, int line, long long t);

iv_handler iv_seteventhandler(iv_handler hproc);
iv_handler iv_restoreeventhandler(iv_handler hproc, int keytm1, int keytm2);
void iv_enqueue(iv_handler hproc, int type, int par1, int par2);
long long iv_get_first_timer();
void iv_process_timers();
char **iv_enum_files(char ***list, const char *path1, const char *path2, const char *path3, const char *extension, int dropext);
int iv_enum_dictionaries(char ***p_dlist, char ***p_fnlist, int onlysystem, int showhidden);
void iv_drawsymbol(int x, int y, int size, int symbol, int color);
irect *iv_windowframe(int x, int y, int w, int h, int bordercolor, int windowcolor, const char *title, int button);
void iv_scrollbar(int x, int y, int w, int h, int percent);
int iv_textblock(int x, int y, int w, unsigned short *p, int len, int color, int angle, int rtlbase);
void iv_stretch(const unsigned char *src, int format, int sx, int sy, int sw, int sh, int srclw,
        int dx, int dy, int dw, int dh, int flags, const unsigned char *mask, int masklw);
void iv_area(int dstx, int dsty, int width, int height,
        const unsigned char *pixels, int srclw,
        const unsigned char *mask, int masklw,
        int srcx, int srcy, int depth);
void iv_draw_hourglass_frame(int x, int y, int frame, int pure);
int iv_msgtop();
void iv_timechanged();
void iv_setsoftupdate();
void iv_actualize_panel(int update);
void iv_update_panel(int with_time);
int iv_panelevent(int *in_type, int *in_x, int *in_y);
void iv_update_orientation(int isexternal);
void iv_check_gsensor();
int iv_transform_key(int key);
void iv_keyswitch_rtl(int *par);
void iv_getkeymapping(keymap_t type, const char *act0[], const char *act1[], const char *act2[], int count);
void iv_getglobalkeymapping(const char *act0[], const char *act1[], const char *act2[], int count);
void iv_rise_poweroff_timer();
void iv_key_timer();
void iv_poweroff_timer();
void iv_panelupdate_timer();
void iv_exit_actions();
void iv_exit_handler();
void iv_setup_touchpanel();
void hw_ts_default(int *a, int *b, int *c, int *d, int *e, int *f, int *nx, int *ny);
int iv_transform_pointer(hw_event *);
void iv_setup_gsensor();
const char *iv_shortpower_action(int times);
int iv_init_profiles();
int iv_use_profiles(int nocache);
int iv_switch_profile(const char *name);
void iv_check_profile();
void iv_invertitem(int x, int y, int w, int h, int d);
void iv_check_uiquery();
void iv_hintmsg(int x, int y, const char *s, ...);
void global_key_action(char *name);
int iv_check_task_messages();
void iv_foreground_event();
void iv_background_event();
void iv_openmainmenu();
void iv_openlastbooks();
void iv_opencalendar();
void iv_toggleplaying();
void iv_pause();
void iv_volumeup();
void iv_volumedown();
void iv_prevtrack();
void iv_nexttrack();
int iv_panel_used();
void init_partner();
void hw_clear_last_update_time(void);
long long hw_get_last_update_time(void);

/**
 * @brief iv_set_system_language_old
 * @param lang - old proprietary format
 */
INKVIEW_DEPRECATED("Use `iv_set_system_locale` instead") void iv_set_system_language_old(const char* lang);

typedef enum
{
    iv_set_system_locale_ok,
    iv_set_system_locale_empty,
    iv_set_system_locale_too_much_parts,
    iv_set_system_locale_contains_unalowed_syms,
    iv_set_system_locale_lang_is_not_iso_639_1,
    iv_set_system_locale_region_is_not_iso_3166_1_alpha_2,
    iv_set_system_locale_unsupported_proprietary_language_code,
    iv_set_system_locale_can_not_open_config,
} iv_set_system_locale_error;

/**
 * @brief iv_set_system_locale - sets system locale in global config
 * @param locale - in format `[lang]_[region]` where `lang` is `iso_639_1` and `region` is `3166_1_alpha_2` (region is optional)
 * @return error
 */
iv_set_system_locale_error iv_set_system_locale(const char* locale);

int pager_height();
int check_rtlbase(unsigned short *p, int len);
unsigned short *rtl_convert_l(unsigned short *s, int len, unsigned short **pos, int *back);
unsigned short *rtl_convert_r(unsigned short *s, int len);
void arabic_shape(unsigned short *buffer, int len, int tashkeel);
unsigned short *arabic_shape_copy(unsigned short *dest, int sourceLength, int tashkeelFlag);

/*
 * function is obsolete, use GetCookies instead
 * may give not full data
 */
int EnumCookie(iv_netcookie *cookie_rec);

/*
 * function is used in pair with EnumCookie wich is obsolete
 */
void EnumCookieFree(iv_netcookie *cookie_rec);

/*
 * Returnes cookie_node chain
 */
const cookie_node * GetCookies();

// encrypt and decrypt data with device specific key
// returns length of output buffer or -1 if error happened
// buffer should be freed by caller
// output buffer larger than input buffer
int EncryptMem(const void *indata, size_t inlength, unsigned char **outdata);
int DecryptMem(const void *indata, size_t inlength, unsigned char **outdata);


/**
 * \brief Set status for obreey sync. Status become available for other processes through iv_get_obreey_status
 * \param new_status status to set. Should be one of the value defined through IV_OSY_STATUS_*
 * Function is not thread safe
 */
void iv_set_obreey_status(int new_status);

/*
 * Function is the collector of network interface info.
 * The interface name that will be collected info transfer in interface value.
 * Returns is pointer to network_interface_info struct, which contain info about assigned interface.
 * May return NULL, if error occurred. If interface is NULL, then using eth0 by default.
 */
network_interface_info *hw_get_network_interface_info (const network_interface *interface);

/*
 * The purpose of func is to get current default gateway for assigned interface.
 * Func can return NULL pointer if error occured. In another case return pointer into
 * string that contain IPv4 numbers-and-dots notation. The result is malloced, and it should be cleared.
 */
network_interface *hw_get_net_gateway_info (const network_interface *interface);

/*
 * The purpose of func is to get current IPv4 addresses, that resolving dns.
 * The results of func are network_interface_array, they contain quantity and array of ip addresses of DNS services
 * Can return NULL if error occurred. The array should be cleared by free after usage.
 */
network_interface_array *hw_get_net_dns_info (void);

/*
 * Get network signal quality for current connection.
 * Return -1 if interface has not been found or error has occurred,
 * otherwise - number of quality in range 0...5
 */
int hw_get_net_signal_quality(void);

/*
 * Returns 128 bit ID that device unique and not known when device manufactured
 */
const char *GetDeviceID(void);

/*
 * Returns PARTNER_ID, no parnter is DEFAULT == 0
 */
PARTNER_ID GetCustomizedPartner(void);

/*
const char* GetCustomizedPartnerName()
- return lowercase name of partner or "PocketBook"
*/
const char* GetCustomizedPartnerName();

/*
const char* GetStoreName()
- return partner store name
*/
const char* GetStoreName();

/*
const char* GetStoreClientId()
- return partner store client id
*/
const char* GetStoreClientId();

/*
 * Kill some stores and restart some applications after change partner
 */
void RestartOnPartnerChanged();

int IsBrandFixed();

typedef enum DeviceConfigFlag_e {
    kDevCfgHasStoreOpds,
    kDevCfgHavePbCloud,
    kDevCfgNativeAuthForWeb,
    kDevCfgNeedAutoRegistrationToCloud,
    kDevHasOpds,
    kDevUseAudio, // Check flag only if (HWC_AUDIO && device_ID() == DEVICE_740). for 740 CIS it should be 0, other device - unknown.
    kDevLegimiOff,
} DeviceConfigFlag;

int getDeviceConfigFlag(DeviceConfigFlag flag);
const char* getBookstoreUrl(const char* def_value);

/*
 * Returns native auth flag
 */
int IsPartnerSupportNativeAuth();

/*
 * Returns auto registration to cloud with store credentials flag
 */
int IsEnabledAutoRegistrationToCloud();

/*
 * Signals to monitor task started and initialized
 */
void SignalTaskStarted(void);

/*
 * Sets drag dead zone in square pixels (i.e. for 20 px drag zone pass 20*20=400 as argument)
 */
void SetTouchDragDeadZone(unsigned int squarepx);

/*
 * Gets drag dead zone in square pixels (i.e. for 20 px drag zone will return 20*20=400 like it was passed in SetTouchDragDeadZone)
 *   default value in TOUCHDRAGDEADZONE macro
 */
unsigned int GetTouchDragDeadZone(void);

ibitmap *bm_make(unsigned char *bmdata, int w, int h);
void bm_dither(unsigned char *data, int sx, int sy, int w, int h, int lw, int levels);

int hw_ussdquery(const char *ussd, char **msg);

/*
 * Check if battery is full by using kernel information if supported or battery SOC if not
 */
int hw_is_battery_full(void);

/*
 * Extend time to sleep will be thrown off.
 */
void hw_force_to_sleep(void);


/*
 * Converts timeval structure to timems
 */
long long timeval2timems(struct timeval *time);

/*
 * Convert RGB565 image to 8-bit greyscale
 */
void iv_rgb565_to_gs(struct iv_img_conv *c);

/*
 * Convert RGB565 image to 8-bit greyscale with gamma correction
 */
void iv_rgb565_to_gs_with_gamma(struct iv_img_conv *c, double gamma);
/*
 * Convert RGB24 image to 8-bit greyscale
 */
void iv_rgb24_to_gs(struct iv_img_conv *c);

/*
 * Convert RGBA32 premultiplied image to 8-bit greyscale
 */
void iv_rgba32premultiplied_to_gs(struct iv_img_conv *c);

/*
 * Convert RGBA32 premultiplied image to RGB888
 */
void iv_rgba32premultiplied_to_rgb888(struct iv_img_conv *c);

/*
 * Convert 4-bit image to 8-bit greyscale
 */
void iv_gs4_to_gs(struct iv_img_conv *c);

/*
 * Copy 8-bit greyscale image
 */
void iv_gs_to_gs(struct iv_img_conv *c);


/*
 * Blend one scanline of ARGB32 premultiplied image onto 8-bit or RGB24 fb
 */
void iv_blend_argb32premul_to_gs8(struct iv_img_conv *c);
void iv_blend_argb32premul_to_rgb24(struct iv_img_conv *c);

//BT client functionality
bt_service_obj *bt_service_init_input(int id);
int bt_service_cancel_input(bt_service_obj *);
int bt_service_close_input(bt_service_obj *);
enum bt_service_e bt_service_get_type(bt_service_obj *obj);

//BT OBEX service
enum obex_status_e obex_get_status(bt_service_obj *);
char *obex_get_filename(bt_service_obj *);
char *obex_get_filepath(bt_service_obj *);
char *obex_get_description(bt_service_obj *);
char *obex_get_mimetype(bt_service_obj *);
long obex_get_filesize(bt_service_obj *);
int obex_set_auth(bt_service_obj *, int auth, char *new_filename, char *new_filepath);
enum obex_status_e obex_get_transfer(bt_service_obj *,long *transferred, long *total);
int hw_init_shm();
void set_fb_orientation(int n);
int hw_flightmode(bool mode);
bool hw_is_flightmode(void);

void init_logger_mutex(void);

typedef enum {
	IOC_CONFIGSTORAGE_OP_CHECK	= 0,
	IOC_CONFIGSTORAGE_OP_READ,
	IOC_CONFIGSTORAGE_OP_WRITE,
} IocConfigStorageOperation_t;

typedef struct {
	IocConfigStorageResult_t result;
	ConfigStorageSlot_t slot;
	ConfigStorageSert sert;
	size_t data_size;
	uint8_t data[];
} IocReqConfigStorage_t;

int hw_ioc_config_storage_write(ConfigStorageSlot_t slot, void *data, size_t data_size, ConfigStorageSert sign);
int hw_ioc_config_storage_read(ConfigStorageSlot_t slot, void *data, size_t *data_size, ConfigStorageSert *sign);
int hw_ioc_config_storage_check(ConfigStorageSlot_t slot, void *data, size_t data_size, ConfigStorageSert sign);


bool isDeviceSupportSecure();
bool isDeviceInitedSecure();

int isDeviceContract();
int isDeviceContractLock();
time_t getDeviceContractNextCheck();
time_t getDeviceContractEnd();

bool hw_is_screen_mode_inverted(void);
void hw_set_screen_mode_inverted(bool state);

int hw_writelowpowerbootlogo(const ibitmap *bm);
int hw_writebootlogo(const ibitmap* bm, int permanent);

// jpegtiff.c
void SetLoadFrameColor(int color);

// file.c
int is_link(const char *in, char *out, int len);

// get panel status as text for play function in accessibility mode
void iv_get_panel_as_accessibility_text(char *buf, int size);

ifont *iv_get_theme_font_scaled(const char *name, const char *deflt);
ifont *iv_get_theme_font_scaled_part(const char *name, const char *deflt, double scaleKoef);

// return new OpenFont
ifont *iv_font_rescale(ifont *font, double koeff);

#ifdef __cplusplus
}
#endif

#endif
